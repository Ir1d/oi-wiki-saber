<template>
  <div class="leftbar" :class="{'is-hidden': false}">
    <div class="sticky">
      <div class="items">
          ...
          <div class="item-children" :class="{'is-expanded': isExpanded(item.children)}">
            <div class="item-child" v-for="(childItem, i) in item.children" :key="i">
              ...
              <div
                class="item-children"
                v-else
                @click="toggleOpenLink(childItem.children[0].link)"
                :class="{'is-expanded': isExpanded(childItem.children)}"
              >
                {{ childItem.title }}
                <div class="item-child" v-for="(childItem2, ii) in childItem.children" :key="ii">
                  <saber-link
                    v-if="childItem2.link"
                    :to="childItem2.link"
                    :class="{active: isActive(childItem2.link)}"
                  >{{ childItem2.title }}</saber-link>
                </div>
              </div>
            </div>
          </div>
